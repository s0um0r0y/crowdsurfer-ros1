...
'''
implementing a batch trajectory planning algorithm for multi-agent navigation
The code uses Bernstein polynomials for trajectory generation, JAX for acceleration, 
and Gaussian trajectory sampling for optimization
'''

class batch_crowd_nav():
    '''
    a_obs_1, b_obs_1, a_obs_2, b_obs_2: Shape parameters for obstacles (likely ellipse or bounding box dimensions).
    v_max, v_min, a_max: Maximum/minimum velocity and acceleration.
    num_obs_1, num_obs_2: Number of obstacle types (1 and 2).
    t_fin: Final time for the trajectory.
    num: Number of discrete time steps.
    num_batch: Batch size for parallel trajectory sampling.
    maxiter, maxiter_cem: Maximum iterations for optimization.
    weight_smoothness, weight_track: Weights for smoothness and tracking accuracy.
    way_point_shape: Shape of the waypoints (could be a geometric shape).
    ...
    def __init__(self, 
        ...
        # t: Discretized time step duration.
        self.t = self.t_fin/self.num
        self.num_batch = num_batch

        # ellite_num_const and ellite_num: Parameters for optimization, 
        # likely for elite sampling in CEM (Cross-Entropy Method).
        ...
        # num_obs_proj: Number of projected obstacles
        self.num_obs_proj = 20

        # Generates a time vector tot_time from 0 to t_fin with num samples.
        tot_time = np.linspace(0, t_fin, num)
        tot_time_copy = tot_time.reshape(num, 1)

        self.P, self.Pdot, self.Pddot = bernstein_coeff_order10.generate_order_10_bernstein_coefficients(10,
                                                                         tot_time_copy[0], 
                                                                         tot_time_copy[-1], 
                                                                         tot_time_copy)
        ...
        # Converts numpy arrays to JAX arrays for faster computation 
        ...
        # Sets up cost function for smoothness using the second derivative
        self.cost_smoothness = self.weight_smoothness*jnp.dot(self.Pddot_jax.T, self.Pddot_jax)	

        # Identity matrix for projection constraints
        self.A_projection = jnp.identity( self.nvar)

        # Equality constraints for start and end points (position, velocity, acceleration)			
        ...
        # Generates a matrix A representing the second-order finite differences (discrete Laplacian).
        self.A_obs = jnp.tile(self.P_jax, (self.num_obs_proj+self.num_obs_2, 1))

        ########## for up sampling

        # Generates a finer grid for up-sampled trajectories (100 samples instead of num)
        # Useful for smoother control and path planning
        ...
        P_up, Pdot_up, Pddot_up = bernstein_coeff_order10.generate_order_10_bernstein_coefficients(10, 
                                                                                                   tot_time_copy_up[0], 
                                                                                                   tot_time_copy_up[-1], 
                                                                                                   tot_time_copy_up)
        ...
        ########################## computing initial covariance for trajectories
        
        # Computes a matrix A representing the second-order finite differences (discrete Laplacian).
        # Helps in penalizing high curvature.
        ...
        # Generates a covariance matrix cov using pseudo-inverse of R.
        ...
        # Samples eps_k from a multivariate Gaussian distribution.
        # Used for trajectory optimization (CEM algorithm).
        eps_k = np.random.multivariate_normal(mu, 0.1*cov, (int(self.num_batch/1), ))
        eps_k_up_sampling = np.random.multivariate_normal(mu, 0.1*cov, 
                                                          (int((self.num_batch * self.initial_up_sampling)), ))
        ...
        '''
        Sets up a cost function considering:
        Path length
        Path smoothness (acceleration)
        Small regularization term (0.0001)
        '''
        self.cost_x =  jnp.dot(self.P_jax.T, self.P_jax) 
        ...
        # regularization parameters or weights
        self.rho_proj = 1
        self.rho_obs = 1
        self.rho_ineq = 1

        # Combines constraints to ensure the trajectory meets dynamic constraints (velocity, acceleration, obstacles).
        ...
        self.cost_mat_x_projection = jnp.vstack((  jnp.hstack(( self.cost_x_projection, self.A_eq.T )), 
                                                 jnp.hstack(( self.A_eq, 
                                                             jnp.zeros(( 
                                                                 jnp.shape(self.A_eq)[0], 
                                                                 jnp.shape(self.A_eq)[0] )) )) ))
        ...
        # scaling factor for the initial trajectory guess
    ...
    # Use JIT compilation for efficient execution on the GPU
    @partial(jit, static_argnums=(0,),backend="gpu")
    def path_spline(self, x_waypoint, y_waypoint):

        # calculates the difference between consecutive elements in the input arrays
        x_diff = jnp.diff(x_waypoint)
        y_diff = jnp.diff(y_waypoint)

        # gives the cumulative sum, representing the distance along the path
        arc = jnp.cumsum(jnp.sqrt( x_diff**2 + y_diff**2))
        # Total path length from start to end
        arc_length = arc[-1]

        # jnp.linspace generates self.way_point_shape points from 0 to arc_length, 
        # representing sampling positions along the path
        # arc_vec = jnp.linspace(0, arc_length, self.way_point_shape)

        num_points = 11  # Explicit integer value
        arc_vec = jnp.linspace(0, arc_length, num_points)

        return arc_length, arc_vec, x_diff, y_diff

    # This method computes the contouring error, which measures how close a target point is 
    # to the path
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_contouring_error(self, x_waypoint, y_waypoint, x_target_point, y_target_point, arc_vec):

        # Calculates Euclidean distance between the target point and each waypoint
        ...
        # The arc length of the closest waypoint to the target point.
        arc_point = arc_vec[index]

        # x_project, y_project: Coordinates of the closest point on the path
    ...
    # Predicts trajectories of dynamic obstacles using velocity and initial positions
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_obs_traj_prediction(self, 
                                    x_obs_init_dy, 
                                    y_obs_init_dy, 
                                    vx_obs_dy, 
                                    vy_obs_dy, 
        ...
        '''
        Computes future obstacle positions using linear motion:
        x_obs_trajectory: Future x-coordinates of obstacles.
        y_obs_trajectory: Future y-coordinates of obstacles.
        _dy variants represent a dynamic obstacle with changing velocity.
        '''
        ...
        # Dist: Distance between robot's initial position and each obstacle's initial position
        Dist = jnp.sqrt( (x_init - x_obs_trajectory[:,0])**2 +(y_init - y_obs_trajectory[:,0])**2) 

        # Returns indices that would sort Dist in ascending order (closest obstacles first).
        idx_dis = jnp.argsort(Dist)

        # Select nearest obstacles based on configuration
        # num_obs_1: Number of obstacles to consider for collision avoidance.
        # num_obs_proj: Subset for projection calculations.
    ...
    # static_argnums=(0,) treats self as a static argument, improving performance.
    @partial(jit, static_argnums=(0,),backend="gpu")	
    def compute_boundary_vec(self, initial_state, x_fin, y_fin):		

        x_init, y_init, vx_init, vy_init, ax_init, ay_init = initial_state

        # This step helps set up constraints for optimization or path planning.
        ...
        # Horizontally stacks vectors to form boundary conditions
    ...
    # This method generates an initial trajectory guess using a warm start
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_warm_traj(self, initial_state, v_des, x_waypoint, y_waypoint, arc_vec, x_diff, y_diff):

        x_init, y_init, vx_init, vy_init, ax_init, ay_init = initial_state

        # Find Closest Waypoint (Start Point)
        dist = jnp.sqrt( (x_waypoint - x_init)**2 + (y_waypoint - y_init)**2 )
        index = jnp.argmin(dist)
        arc_point = arc_vec[index]

        # Look-Ahead to Determine Final Path Point
        look_ahead_point_path = arc_point+v_des*self.t_fin

        # Clips to ensure the point stays within path boundaries.
        look_ahead_point_path = jnp.clip(look_ahead_point_path, arc_vec[0], arc_vec[-1])

        # Finds the closest waypoint to this look-ahead point, representing the final position
        ...
        # Look-Ahead for Midpoint (Halfway)
        ...
        # Determines the normal vector at the midpoint for generating a better guess.
        normal_x =  -1*(ydot_m)/jnp.sqrt(xdot_m**2 + ydot_m**2  )
        normal_y =   1*(xdot_m)/jnp.sqrt(xdot_m**2 + ydot_m**2  )

        # scale_factor_warm determines offset magnitude
        ...
        A_eq = jnp.vstack(( self.P_jax[0], 
                           self.Pdot_jax[0], 
                           self.Pddot_jax[0], 
                           self.P_jax[50], 
                           self.P_jax[-1]  ))

        cost_mat = jnp.vstack((  jnp.hstack(( jnp.dot(self.Pddot_jax.T, self.Pddot_jax), A_eq.T )), 
                               jnp.hstack(( A_eq, jnp.zeros(( jnp.shape(A_eq)[0], 
                                                             jnp.shape(A_eq)[0] )) )) ))
    ...
    # This method computes a trajectory guess for the robot, considering waypoints and obstacles.
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_traj_guess(self, 
                           initial_state, 
                           x_obs_trajectory, 
                           y_obs_trajectory, 
                           x_obs_trajectory_dy, 
        ...
        # Computes the Euclidean distance from the initial state to all waypoints.
        dist = jnp.sqrt((x_waypoint - x_init)**2 + (y_waypoint - y_init)**2 )
        index = jnp.argmin(dist)

        # Finds the closest waypoint index (index) and retrieves the corresponding arc_point on the arc vector.
        arc_point = arc_vec[index]

        '''
        Look-Ahead Point Calculation:
        Predicts a point on the arc vector based on the desired velocity (v_des) and total time (t_fin).
        Clamps it within the bounds of arc_vec using jnp.clip.
        Finds the closest index (index_final_path) to this look-ahead point.
        '''
        look_ahead_point_path = arc_point+v_des*self.t_fin
        look_ahead_point_path = jnp.clip(look_ahead_point_path, arc_vec[0], arc_vec[-1])
        index_final_path = jnp.argmin(jnp.abs(look_ahead_point_path-arc_vec)  )
        ...
        # Constructs constraint matrices for optimization
        ...
        # Solves a quadratic optimization problem to minimize trajectory error.
        A_eq = jnp.vstack(( self.P_jax[0], 
                           self.Pdot_jax[0], 
                           self.Pddot_jax[0],  
                           self.P_jax[25],  
        ...
        # self.P_jax represents a basis matrix or trajectory matrix used for interpolation or fitting a curve to the solution points.
        ...
        # Computes the relative distance between each point in the sampled trajectory and all static and dynamic obstacles.

        # jnp.newaxis adds an extra dimension, enabling broadcasting for element-wise operations.
        ...
        # Represents a constraint on obstacle avoidance using a quadratic form.
        ...
        # Values of dist_obs > 0 imply safe distances, while ≤ 0 indicates collision or near-collision.
        dist_obs = jnp.hstack(( dist_obs_st, dist_obs_dy ))

        # Calculates penalties for obstacle violations using the Euclidean norm.
        cost_obs_penalty = jnp.linalg.norm(jnp.maximum(jnp.zeros(( self.initial_up_sampling*self.num_batch, 
                                                                  self.num*(self.num_obs_1 + self.num_obs_2)  )), 
                                                                  dist_obs), 
                                                                  axis = 1)# jnp.linalg.norm(1.6*jnp.minimum(jnp.zeros(( self.initial_up_sampling*self.num_batch, self.num*self.num_obs  )), -dist_obs), axis = 1)

        # returns indices that sort the penalty values in ascending order
        idx_ellite = jnp.argsort(cost_obs_penalty)

        # Selects the top samples with the least penalty (most feasible trajectories)
        ...
        # Combines the elite samples with precomputed guesse
        x_guess = jnp.vstack(( x_guess_sample, x_guess_per))
        y_guess = jnp.vstack(( y_guess_sample, y_guess_per))

        # Computes the linear cost components using the basis matrix P_jax
        ...
        # Computes the mean and covariance of the solution, useful for uncertainty estimation
    ...
    # This function computes a projection step for trajectory optimization, 
    # using obstacle avoidance constraints and optimization matrices.
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_projection(self, 
        ...
        # These variables represent the starting point for the projection operation.
        b_projection_x = sol_x_bar
        b_projection_y = sol_y_bar

        '''
        rad_a and rad_b store obstacle radii for ellipsoid-shaped obstacles:
        self.a_obs_1 and self.b_obs_1: Semi-major and semi-minor axes for static obstacles.
        self.a_obs_2 and self.b_obs_2: Semi-major and semi-minor axes for dynamic obstacles.
        '''
        rad_a = jnp.hstack(( self.a_obs_1 * 
                            jnp.ones(( self.num_batch,  self.num*(self.num_obs_proj) )), 
                            self.a_obs_2 * jnp.ones(( self.num_batch,  self.num*(self.num_obs_2) )) ))
        rad_b = jnp.hstack(( self.b_obs_1 * 
                            jnp.ones(( self.num_batch,  self.num*(self.num_obs_proj) )), 
                            self.b_obs_2 * jnp.ones(( self.num_batch,  self.num*(self.num_obs_2) )) ))

        # represents the obstacle projection in the x-direction based on distance d_obs, 
        # angle alpha_obs, and radii rad_a
        temp_x_obs = d_obs*jnp.cos(alpha_obs)*rad_a
        
        # b_obs_x stores the projected obstacle boundaries in the x-direction
        b_obs_x = jnp.hstack(( x_obs_trajectory.reshape(self.num*self.num_obs_proj ) , 
                              x_obs_trajectory_dy.reshape(self.num*self.num_obs_2 ) ))+temp_x_obs
            
        temp_y_obs = d_obs*jnp.sin(alpha_obs)*rad_b
        b_obs_y = jnp.hstack(( y_obs_trajectory.reshape(self.num*self.num_obs_proj ) 
                              , y_obs_trajectory_dy.reshape(self.num*self.num_obs_2 ) ))+temp_y_obs
        
        # Acceleration constraints in x and y directions using distance d_a
        b_ax_ineq = d_a*jnp.cos(alpha_a)
        b_ay_ineq = d_a*jnp.sin(alpha_a)

        # Velocity constraints in x and y directions using distance d_v 
        b_vx_ineq = d_v*jnp.cos(alpha_v)
        b_vy_ineq = d_v*jnp.sin(alpha_v)
        
        # These expressions compute the linear costs for the projection step
        # rho_proj, rho_obs, and rho_ineq are regularization parameters or weights
        lincost_x =  - self.rho_proj * jnp.dot(self.A_projection.T, b_projection_x.T).T 
        -lamda_x-self.rho_obs*jnp.dot(self.A_obs.T, b_obs_x.T).T
        -self.rho_ineq*jnp.dot(self.A_acc.T, b_ax_ineq.T).T
        -self.rho_ineq*jnp.dot(self.A_vel.T, b_vx_ineq.T).T
        lincost_y =  - self.rho_proj * jnp.dot(self.A_projection.T, b_projection_y.T).T 
        -lamda_y-self.rho_obs*jnp.dot(self.A_obs.T, b_obs_y.T).T
        ...
        # store the final optimized control variables
    ...
    # This method computes the initial alpha values for the trajectory optimization problem.
    # It calculates the relative distances to obstacles and the maximum allowable velocities and accelerations.
    @partial(jit, static_argnums=(0,),backend="gpu")	
    def initial_alpha_d(self, 
                        x_guess, 
                        y_guess, 
                        xdot_guess, 
                        ydot_guess, 
        ...
        '''
        rad_a and rad_b represent the semi-major and semi-minor axes of ellipsoid-shaped obstacles:
        self.a_obs_1 and self.a_obs_2 are radii for static and dynamic obstacles in the x-direction.
        self.b_obs_1 and self.b_obs_2 are radii for static and dynamic obstacles in the y-direction.
        '''
        rad_a = jnp.hstack(( self.a_obs_1 * jnp.ones(( self.num_batch,  self.num*(self.num_obs_proj) )), 
                            self.a_obs_2 * jnp.ones(( self.num_batch,  self.num*(self.num_obs_2) )) ))
        rad_b = jnp.hstack(( self.b_obs_1 * jnp.ones(( self.num_batch,  self.num*(self.num_obs_proj) )), 
                            self.b_obs_2 * jnp.ones(( self.num_batch,  self.num*(self.num_obs_2) )) ))

        # alpha_obs represents the angle between the current trajectory and obstacles using the obstacle radii.
        alpha_obs = jnp.arctan2( ws_alpha* rad_a, wc_alpha* rad_b)

        # c1_d and c2_d are coefficients for calculating distances based on the obstacle shape and relative position
        ...
        # d_obs represents the distance to obstacles
        d_obs = c2_d/c1_d

        # ensures d_obs is not less than 1, preventing division errors and maintaining safe distance
        ...
        # alpha_v is the direction of velocity using guessed velocities 
        ...
        # d_v represents the maximum allowable velocity along alpha_v, clipped by v_max
        ...
        # alpha_a is the direction of acceleration using guessed accelerations
        ...
        # d_a represents the maximum allowable acceleration along alpha_a, clipped by a_max.
        d_a = c2_d_a/c1_d_a
        d_a = jnp.minimum(self.a_max*jnp.ones((self.num_batch, self.num)), d_a )

        #########################################

        # Residuals res_ax_vec, res_vx_vec, etc., quantify constraint violations
        ...
        # lamda_x and lamda_y are updated using these residuals to refine the optimization process
    ...
    def compute_alph_d_proj(self, 
        ...
        # wc_alpha_temp and ws_alpha_temp represent the relative position of the agent to the obstacles in the x and y directions, respectively
        ...
        # the angle alpha_obs between agent and obstacle based on the relative position
        alpha_obs = jnp.arctan2( ws_alpha* rad_a, wc_alpha* rad_b)

        # c1_d and c2_d are intermediate terms used to calculate the distance
        c1_d = 1.0*self.rho_obs*(rad_a**2*jnp.cos(alpha_obs)**2 + rad_b**2*jnp.sin(alpha_obs)**2 )
        c2_d = 1.0*self.rho_obs*(rad_a*wc_alpha*jnp.cos(alpha_obs) + rad_b*ws_alpha*jnp.sin(alpha_obs)  )
        ...
        # residuals represent constraint violations
    ...
    # This function performs iterative projection optimization using Lagrangian relaxation for constrained trajectory sampling
    @partial(jit, static_argnums=(0, ))	
    def compute_projection_sampling(self, 
                                    key, 
                                    sol_x_bar, 
                                    sol_y_bar, 
        ...
        # Computes equality constraints for the optimization problem, ensuring the trajectory 
        # meets initial and final state conditions
        b_eq_x, b_eq_y = self.compute_boundary_vec(initial_state, x_fin, y_fin)

        # initial Lagrangian multipliers (lamda_x, lamda_y) for constraint handling
        alpha_obs_init, d_obs_init, alpha_a_init, d_a_init, alpha_v_init, d_v_init, lamda_x_init, lamda_y_init = self.initial_alpha_d(x_guess, 
                                                                                                                                      y_guess,  
                                                                                                                                      xdot_guess, 
                                                                                                                                      ydot_guess, 
                                                                                                                                      xddot_guess, 
        ...
        # Inner Projection Function using Lax Scan
        def lax_projection(carry, proj_iter ):
            ...
            # Update trajectory projections based on constraints
            c_x, c_y, x, y, xdot, ydot, xddot, yddot = self.compute_projection(x_obs_trajectory_proj, 
                                                                               y_obs_trajectory_proj, 
                                                                               x_obs_trajectory_dy, 
                                                                               y_obs_trajectory_dy, 
                                                                               d_obs, 
            ...
            # Recalculate projection constraints
            alpha_obs, d_obs, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, res_norm_batch = self.compute_alph_d_proj( x, 
                                                                                                                      y, 
                                                                                                                      xdot, 
                                                                                                                      ydot, 
                                                                                                                      xddot, 
        ...
        # Initializes carry values for lax.scan with zeros and initial constraint values
        carry_init =  jnp.zeros((self.num_batch,self.nvar)), 
        ...
        # Returns the final state (carry_fin) and all intermediate projections (result)
    ...
    def compute_cost_batch(self, 
        ...
        # Calculates the distance between the current position and obstacle positions.
        ...
        # Reshapes distances to a flattened format for efficient calculations
        wc_alpha = wc_alpha_temp.transpose(1, 0, 2)
        ws_alpha = ws_alpha_temp.transpose(1, 0, 2)
        ...
        # Computes individual cost components and combines them into a final cost value
        clearance_cost = - jnp.min(dist_obs, axis = 1)
        cost_obs = jnp.linalg.norm(jnp.maximum(jnp.zeros(( self.ellite_num_const, 
                                                          self.num*(self.num_obs_1+self.num_obs_2))), 
                                                          dist_obs), 
                                                          axis = 1)
    ...
    def comp_prod(self, diffs, d ):
        ...
        # Adds a new axis, transforming diffs from shape (num_samples, nvar) to (num_samples, 1, nvar)
        term_1 = jnp.expand_dims(diffs, axis = 1)

        # Adds a new axis, resulting in shape (1, num_samples, nvar)
        term_2 = jnp.expand_dims(diffs, axis = 0)

        # jnp.outer(term_1, term_2): Computes the outer product of term_1 and term_2, 
        # resulting in a 4D tensor of shape (num_samples, num_samples, nvar, nvar)
        # prods is a 4D tensor of shape (num_samples, num_samples, nvar, nvar) containing weighted outer products
    ...
    # This function performs cross-entropy optimization (CEM) to generate new samples based on the cost function. 
    # It adjusts the mean and covariance based on the best samples (elite set)
    @partial(jit, static_argnums=(0,))	
    def compute_shifted_samples(self, 
                                key, 
                                cost_batch, 
                                c_x_ellite, 
                                c_y_ellite, 
        ...
        '''
        c_ellite: Combines c_x_ellite and c_y_ellite horizontally, resulting in shape (num_batch, 2*nvar).
        beta_param = jnp.min(cost_batch): Finds the minimum cost in the batch.
        d: Exponential weighting based on the cost difference to encourage better samples.
        sum_d: Normalization factor for weights.
        c_mean: Weighted moving average to update the mean
        '''
        ...
        '''
        diffs: Difference between samples and mean.
        self.vec_product(diffs, d): Computes weighted outer products using comp_prod to calculate covariance.
        c_cov: Updated covariance using a weighted average.
        '''
        ...
        # Constraints ensure samples stay away from obstacles.
        # Calculates penalties based on distances and sorts based on cost.
        dist_obs_st = -wc_alpha**2/(self.a_obs_1**2)-ws_alpha**2/(self.b_obs_1**2)+1
        dist_obs_dy = -wc_alpha_dy**2/(self.a_obs_2**2)-ws_alpha_dy**2/(self.b_obs_2**2)+1
    ...
    @partial(jit, static_argnums=(0, ))
    # This function computes control inputs for a robotic system based on the best control coefficients c_x_best and c_y_best
    def compute_controls(self, c_x_best, c_y_best,):

        '''
        jnp.dot: Performs matrix multiplication using JAX (similar to NumPy).
        xdot_best and ydot_best represent velocity profiles in x and y directions.
        xddot_best and yddot_best represent acceleration profiles in x and y directions.
        self.Pdot_up_jax and self.Pddot_up_jax are derivative matrices (differentiating coefficients).
        '''
        ...
        '''
        vx_control and vy_control are the average velocities in x and y directions.
        ax_control and ay_control are the average accelerations in x and y directions.
        self.num_sample specifies the sampling range.
        '''
        ...
        '''
        norm_v_t calculates the magnitude of the velocity vector (speed).
        angle_v_t calculates the orientation angle using arctan2, which returns the angle in radians based on the velocity components.
        '''
    ...
    # This function implements the Cross-Entropy Method (CEM), an optimization technique for trajectory generation.
    @partial(jit, static_argnums=(0, ))	
    def compute_cem(self, 
        ...
        for i in range(0,self.maxiter_cem):
            ...
            c_x, c_y, x, xdot, xddot, y, ydot, yddot, res_norm_batch = self.compute_projection_sampling(key, 
                                                                                                        sol_x_bar, 
                                                                                                        sol_y_bar, 
                                                                                                        x_obs_trajectory_proj, 
                                                                                                        y_obs_trajectory_proj, 
            ...
            # idx_ellite_projection sorts samples based on cost (residual norm)
            # Extracts top elite samples
            ...
            # Flattens elite samples and projects them onto waypoints
            x_guess_flat = x_ellite.reshape(self.ellite_num_const * self.num)
            y_guess_flat = y_ellite.reshape(self.ellite_num_const * self.num)

            normal_distance, x_project, y_project = self.vectorized_projection(x_waypoint,  
                                                                               y_waypoint, 
                                                                               x_guess_flat, 
                                                                               y_guess_flat, 
                                                                               arc_vec)
            ...
            # Computes cost and updates distribution using elite samples
            cost_batch  = self.compute_cost_batch(x_ellite, 
                                                  y_ellite, 
                                                  xdot_ellite, 
                                                  ydot_ellite, 
                                                  xddot_ellite, 
            ...
            sol_x_bar, sol_y_bar, x_guess, y_guess, xdot_guess, ydot_guess, xddot_guess, yddot_guess, c_mean_prev, c_cov_prev = self.compute_shifted_samples(key, 
                                                                                                                                                             cost_batch, 
                                                                                                                                                             c_x_ellite, 
                                                                                                                                                             c_y_ellite,  
                                                                                                                                                             x_obs_trajectory, 
