...
import bernstein_coeff_order10 as bernstein_coeff_order10_arbitinterval
import time
...
class batch_crowd_nav():
    ...
    def __init__(self, a_obs_1, b_obs_1, a_obs_2, b_obs_2, v_max, v_min, a_max, num_obs_1, num_obs_2, t_fin, num, num_batch, maxiter, maxiter_cem, weight_smoothness, weight_track, way_point_shape, v_des):
        ...
        # ellite_num_const and ellite_num: Parameters for optimization, likely for elite sampling in CEM (Cross-Entropy Method).
    ...
    def path_spline(self, x_waypoint, y_waypoint):
        ...
        # jnp.linspace generates self.way_point_shape points from 0 to arc_length, representing sampling positions along the path
        arc_vec = jnp.linspace(0, arc_length, self.way_point_shape)

        return arc_length, arc_vec, x_diff, y_diff

    # This method computes the contouring error, which measures how close a target point is to the path
    @partial(jit, static_argnums=(0,),backend="gpu")
    ...
    # This function computes a projection step for trajectory optimization, using obstacle avoidance constraints and optimization matrices.
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_projection(self, x_obs_trajectory, y_obs_trajectory, x_obs_trajectory_dy, y_obs_trajectory_dy, d_obs, alpha_obs, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, b_eq_x, b_eq_y, sol_x_bar, sol_y_bar ):
        ...
        # represents the obstacle projection in the x-direction based on distance d_obs, angle alpha_obs, and radii rad_a
    ...
    def compute_projection_sampling(self, key, sol_x_bar, sol_y_bar, x_obs_trajectory_proj, y_obs_trajectory_proj, x_obs_trajectory_dy, y_obs_trajectory_dy, lamda_x, lamda_y, x_guess, y_guess,  xdot_guess, ydot_guess, xddot_guess, yddot_guess, initial_state, x_fin, y_fin ):
        ...
        # Computes equality constraints for the optimization problem, ensuring the trajectory meets initial and final state conditions
    ...
    def comp_prod(self, diffs, d ):
        ...
        # jnp.outer(term_1, term_2): Computes the outer product of term_1 and term_2, resulting in a 4D tensor of shape (num_samples, num_samples, nvar, nvar)
