...
class ProjectionGuidance:
    def __init__(
        ...
        self.DEVICE = device
        self.DT = torch.linspace(0, total_time, num_timesteps).to(
            self.DEVICE
        )  # shape (num_timesteps,)
        ...
        ) = bernstein_coeff_order10(tmin=0, tmax=total_time, t_actual=self.DT)
        ...
        INITIAL_COST_MATRIX = (
            ...
            + (
                1
                * self.BERNSTEIN_POLYNOMIALS_SECOND_DIFFERENTIAL.T
                @ self.BERNSTEIN_POLYNOMIALS_SECOND_DIFFERENTIAL
            )
        )  # shape (11, 11)

        self.INVERSE_INITIAL_COST_MATRIX_X = cast(
            Tensor, torch.linalg.inv(INITIAL_COST_MATRIX)
        )  # shape (11, 11)
        ...
        PROJECTION_COST_MATRIX_X = (
            ...
            + self.RHO_COLLISION
            * (self.A_COLLISION.reshape(-1, 11).T @ self.A_COLLISION.reshape(-1, 11))
            ...
            [
                ...
                torch.concatenate(
                    [
                        self.A_EQUALITY,
                        torch.zeros(
                            (self.A_EQUALITY.shape[0], self.A_EQUALITY.shape[0])
                        ).to(device),
                    ],
                    dim=1,
                ),  # shape (4, 15)
        ...
        self.INVERSE_PROJECTION_COST_MATRIX_X = cast(
            Tensor, torch.inverse(PROJECTION_COST_MATRIX_X)
        )  # shape (15, 15)
    ...
    def predict_obstacle_trajectories(
        ...
        obstacle_positions_trajectory_x = (
            obstacle_positions_x[:, :, None]
            + obstacle_velocities_x[:, :, None] * self.DT[None, None, :]
        )  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)

        obstacle_positions_trajectory_y = (
            obstacle_positions_y[:, :, None]
            + obstacle_velocities_y[:, :, None] * self.DT[None, None, :]
        )  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)

        distances = torch.sqrt(
            (initial_ego_position_x[:, None, None] - obstacle_positions_trajectory_x)
            ** 2
            + (initial_ego_position_y[:, None, None] - obstacle_positions_trajectory_y)
            ** 2
        )  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)
        ...
        return (
            obstacle_positions_trajectory_x.take(
                distances
            ),  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)
            obstacle_positions_trajectory_y.take(
                distances
            ),  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)
        )
    ...
    def generate_alpha_d_lambda(
        ...
        collision_d_vector = (
            ...
            * (
                (
                    self.OBSTACLE_ELLIPSE_SEMI_MINOR_AXIS
                    * ego_to_obstacle_distance_x
                    * torch.cos(collision_alpha_vector)
                )
                + (
            ...
            * (
                (
                    (self.OBSTACLE_ELLIPSE_SEMI_MAJOR_AXIS**2)
                    * (torch.sin(collision_alpha_vector) ** 2)
                )
                + (
                    (self.OBSTACLE_ELLIPSE_SEMI_MINOR_AXIS**2)
                    * (torch.cos(collision_alpha_vector) ** 2)
                )
            )
        )  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)

        collision_d_vector = torch.clip(
            collision_d_vector, min=1
        )  # shape (BATCH_SIZE, NUM_OBSTACLES, num_timesteps)
        ...
        velocity_d_vector = (
            ...
            * self.RHO_INEQUALITIES
            * (
                torch.cos(velocity_alpha_vector) ** 2
                + torch.sin(velocity_alpha_vector) ** 2
            )
        )  # shape (BATCH_SIZE, num_timesteps)

        velocity_d_vector = torch.clip(
            velocity_d_vector, max=maximum_velocity
        )  # shape (BATCH_SIZE, num_timesteps)

        resulting_velocity_e_vector_x = (
            ego_velocity_trajectory_x
            - velocity_d_vector * torch.cos(velocity_alpha_vector)
        )  # shape (BATCH_SIZE, num_timesteps)

        resulting_velocity_e_vector_y = (
            ego_velocity_trajectory_y
            - velocity_d_vector * torch.sin(velocity_alpha_vector)
        )  # shape (BATCH_SIZE, num_timesteps)
        ...
        acceleration_d_vector = (
            ...
            * self.RHO_INEQUALITIES
            * (
                torch.cos(acceleration_alpha_vector) ** 2
                + torch.sin(acceleration_alpha_vector) ** 2
            )
        )  # shape (BATCH_SIZE, num_timesteps)
        ...
        resulting_acceleration_e_vector_x = (
            ...
            - acceleration_d_vector * torch.cos(acceleration_alpha_vector)
        )  # shape (BATCH_SIZE, num_timesteps)

        resulting_acceleration_e_vector_y = (
            ego_acceleration_trajectory_y
            - acceleration_d_vector * torch.sin(acceleration_alpha_vector)
        )  # shape (BATCH_SIZE, num_timesteps)
        ...
        updated_lambda_x = (
            ...
            - self.RHO_COLLISION
            * (
                self.A_COLLISION.reshape(-1, 11).T
                @ resulting_collision_e_vector_x.flatten(start_dim=1).T
            ).T
            - self.RHO_INEQUALITIES
            * (self.A_VELOCITY.T @ resulting_velocity_e_vector_x.T).T
            - self.RHO_INEQUALITIES
            * (self.A_ACCELERATION.T @ resulting_acceleration_e_vector_x.T).T
        )  # shape (BATCH_SIZE, 11)
        ...
        updated_lambda_y = (
            ...
            * (
                self.A_COLLISION.reshape(-1, 11).T
                @ resulting_collision_e_vector_y.flatten(start_dim=1).T
            ).T
            - self.RHO_INEQUALITIES
            * (self.A_VELOCITY.T @ resulting_velocity_e_vector_y.T).T
            - self.RHO_INEQUALITIES
            * (self.A_ACCELERATION.T @ resulting_acceleration_e_vector_y.T).T
        )  # shape (BATCH_SIZE, 11)
    ...
    def coefficients_to_trajectory(
        ...
        Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor], Tuple[Tensor, Tensor]
    ]: ...
    ...
    def project_initial_solution_to_trajectory(
        ...
        velocity_e_vector_x = velocity_d_vector * torch.cos(
            velocity_alpha_vector
        )  # shape (BATCH_SIZE, num_timesteps)
        velocity_e_vector_y = velocity_d_vector * torch.sin(
            velocity_alpha_vector
        )  # shape (BATCH_SIZE, num_timesteps)
        ...
        linear_cost_x = (
            -1
            * self.RHO_PROJECTION
            * (self.A_PROJECTION.T @ ego_position_solution_x.T).T
            - lambda_x
            - self.RHO_COLLISION
            * (
                self.A_COLLISION.reshape(-1, 11).T
                @ collision_e_vector_x.flatten(start_dim=1).T
            ).T
            - self.RHO_INEQUALITIES * (self.A_VELOCITY.T @ velocity_e_vector_x.T).T
            - self.RHO_INEQUALITIES
            * (self.A_ACCELERATION.T @ acceleration_e_vector_x.T).T
        )  # shape (BATCH_SIZE, 11)
        linear_cost_y = (
            -1
            * self.RHO_PROJECTION
            * (self.A_PROJECTION.T @ ego_position_solution_y.T).T
            - lambda_y
            - self.RHO_COLLISION
            * (
                self.A_COLLISION.reshape(-1, 11).T
                @ collision_e_vector_y.flatten(start_dim=1).T
            ).T
            - self.RHO_INEQUALITIES * (self.A_VELOCITY.T @ velocity_e_vector_y.T).T
            - self.RHO_INEQUALITIES
            * (self.A_ACCELERATION.T @ acceleration_e_vector_y.T).T
        )  # shape (BATCH_SIZE, 11)

        primal_solution_x = (
            self.INVERSE_PROJECTION_COST_MATRIX_X
            @ torch.concatenate(
                [-1 * linear_cost_x, boundary_conditions_x], dim=1
            ).T  # shape (15, BATCH_SIZE)
        ).T  # shape (BATCH_SIZE, 15)
        primal_solution_y = (
            self.INVERSE_PROJECTION_COST_MATRIX_Y
            @ torch.concatenate(
                [-1 * linear_cost_y, boundary_conditions_y], dim=1
            ).T  # shape (15, BATCH_SIZE)
        ).T  # shape (BATCH_SIZE, 15)
    ...
    def convert_warm_trajectory_to_initial_solution(
        ...
        initial_solution_x = (
            self.INVERSE_INITIAL_COST_MATRIX_X @ -linear_cost_x.T
        ).T  # shape (BATCH_SIZE, 11)

        initial_solution_y = (
            self.INVERSE_INITIAL_COST_MATRIX_Y @ -linear_cost_y.T
        ).T  # shape (BATCH_SIZE, 11)
    ...
    def project_trajectory(
        ...
        maximum_velocity: float,
        maximum_acceleration: float,
        warm_ego_position_trajectory_x: Optional[
            Tensor
        ] = None,  # shape (BATCH_SIZE, num_timesteps)
        warm_ego_position_trajectory_y: Optional[
            Tensor
        ] = None,  # shape (BATCH_SIZE, num_timesteps)
        ...
        assert (
            warm_ego_position_trajectory_x is not None
            and warm_ego_position_trajectory_y is not None
        ) or (initial_solution_x is not None and initial_solution_y is not None), (
            "Either warm_ego_position_trajectory or initial_solution must be provided"
        )

        assert (
            warm_ego_position_trajectory_x is None
            and warm_ego_position_trajectory_y is None
        ) or (initial_solution_x is None and initial_solution_y is None), (
        ...
        lambda_x = torch.zeros((initial_solution_x.shape[0], 11)).to(
            self.DEVICE
        )  # shape (BATCH_SIZE, 11)
        lambda_y = torch.zeros((initial_solution_x.shape[0], 11)).to(
            self.DEVICE
        )  # shape (BATCH_SIZE, 11)
        ...
        return (
            ...
            outputs[-1].ego_position_trajectory_y[
                -1
            ],  # shape (BATCH_SIZE, num_timesteps)
            outputs[-1].ego_velocity_trajectory_x[
                -1
            ],  # shape (BATCH_SIZE, num_timesteps)
            outputs[-1].ego_velocity_trajectory_y[
                -1
            ],  # shape (BATCH_SIZE, num_timesteps)
            outputs[-1].ego_acceleration_trajectory_x[
                -1
            ],  # shape (BATCH_SIZE, num_timesteps)
            outputs[-1].ego_acceleration_trajectory_y[
                -1
            ],  # shape (BATCH_SIZE, num_timesteps)
            outputs[
                -1
            ].error_residuals,  # shape (MAX_PROJECTION_ITERATIONS, BATCH_SIZE,)
        )
