...
from . import variable_cost_functions
from .bernstein_coeff_order10 import generate_order_10_bernstein_coefficients

class CustomSwitchInput(NamedTuple):
    index: int
    cost_weight: float
    arguments: variable_cost_functions.CommonInputs


@jit
def custom_switch(inputs: CustomSwitchInput):
    return inputs.cost_weight * jax.lax.switch(
        index=inputs.index,
        branches=variable_cost_functions.COST_FUNCTION_LIST,
        operand=inputs.arguments,
    )
...
class PriestPlannerCore:
    def __init__(
        ...
        self.vectorized_projection = jit(
            jax.vmap(
        ...
        self.scale_factor = np.random.normal(
    ...
    def compute_traj_guess(
        ...
        A_eq = jnp.vstack(
            (
                ...
                self.Pdot_jax[0],
                self.Pddot_jax[0],
                self.P_jax[int(0.25 * self.num)],
                self.P_jax[int(0.75 * self.num)],
                self.P_jax[int(0.5 * self.num)],
                self.P_jax[-1],
            )
        )
        ...
        cost_obs_penalty = jnp.linalg.norm(
            jnp.maximum(
                jnp.zeros(
                    (
                        self.initial_up_sampling * self.num_batch,
    ...
    def compute_projection(
        ...
        b_obs_x = (
            jnp.hstack(
                (
                    x_obs_trajectory.reshape(self.num * self.num_obs_proj),
        ...
        b_obs_y = (
            jnp.hstack(
                (
                    y_obs_trajectory.reshape(self.num * self.num_obs_proj),
        ...
        lincost_x = (
            -self.rho_proj * jnp.dot(self.A_projection.T, b_projection_x.T).T
            - lamda_x
        ...
        lincost_y = (
            -self.rho_proj * jnp.dot(self.A_projection.T, b_projection_y.T).T
            - lamda_y
    ...
    def initial_alpha_d(
        ...
        d_obs = c2_d / c1_d
        d_obs = jnp.maximum(
            jnp.ones((self.num_batch, self.num * (self.num_obs_proj + self.num_obs_2))),
            d_obs,
        )
    ...
    def compute_cost_batch(
        ...
        x_obs_trajectory_dy,
        y_obs_trajectory_dy,
        variable_cost_function_indices: List[Tuple[int, float]],
    ):
        ...
        common_inputs = variable_cost_functions.CommonInputs(
            cost_batch=cost_batch,
            x=x,
            y=y,
            xdot=xdot,
            ydot=ydot,
            x_obs_trajectory_dy=x_obs_trajectory_dy,
            y_obs_trajectory_dy=y_obs_trajectory_dy,
        )

        return jnp.zeros(50) + jnp.sum(
            jnp.stack(
                [
                    custom_switch(CustomSwitchInput(index=index, cost_weight=weight, arguments=common_inputs))
                    for (index, weight) in variable_cost_function_indices
                ]
            )
        )
    ...
    def compute_cem(
        ...
        c_mean,
        c_cov,
        variable_cost_function_indices,
    ):
        c_mean_prev = c_mean
        c_cov_prev = c_cov
